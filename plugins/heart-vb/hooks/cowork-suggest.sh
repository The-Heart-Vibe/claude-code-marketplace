...
cat <<EOF
{
  "hookSpecificOutput": {
    ...
    "additionalContext": "🤖 [Cowork Spawn Router] Multi-entity task detected (signals: numeric=$NUMERIC_HITS iteration=$ITER_HITS scenario=$SCENARIO_HITS comparative=$COMP_HITS sections=$SECTION_HITS explicit=$EXPLICIT_HITS, N hint: $MAX_N). STRONG RECOMMENDATION: użyj skilla **heart-orchestrate** który auto-aplikuje pełen wzorzec: decompose → spawn ${N} workers (Agent tool, model:'sonnet') w stylu '${WORKFLOW}' → synthesize w main (Opus). Analityk NIE powinien świadomie wybierać modeli ani spawnowac ręcznie — orchestrator robi to za niego. Jeśli z jakiegoś powodu wolisz manualny pattern: Agent({subagent_type: 'general-purpose', model: 'sonnet', description, prompt}) × ${N} parallel + synthesize w main. NIE używaj cowork/orchestrate dla: pojedyncza decyzja (council solo), sequential reasoning, single lookup, final synthesis. Opt-out: prefix 'BEZ COWORK:' lub 'SINGLE SESSION:'. Pełne patterns: skills/heart-custom/heart-orchestrate/SKILL.md."
  }
}
EOF
