...
cat <<EOF
{
  "hookSpecificOutput": {
    ...
    "additionalContext": "🤖 [Cowork Spawn Router] Multi-entity task detected (signals: numeric=$NUMERIC_HITS iteration=$ITER_HITS scenario=$SCENARIO_HITS comparative=$COMP_HITS sections=$SECTION_HITS explicit=$EXPLICIT_HITS, N hint: $MAX_N). RECOMMENDATION: spawn ${N} cowork agents w stylu '${WORKFLOW}'. Każdy agent obsługuje 1 entity z listy. Use Agent tool z 'model: \"sonnet\"' dla workerów (orchestrator main pozostaje na opus). Pattern: Agent({subagent_type: 'general-purpose', model: 'sonnet', description, prompt}) × ${N}, parallel. Po finish — synthesize w main. NIE używaj cowork dla: pojedyncza decyzja/council, sequential reasoning (krok 1 zależy od kroku 0), single lookup, final synthesis. Opt-out per-prompt: prefix 'BEZ COWORK:' lub 'SINGLE SESSION:'. Pattern details w collections/venture-builder.md → 'Cowork patterns dla analityków VB'."
  }
}
EOF
